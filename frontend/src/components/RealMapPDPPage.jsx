  ...
  return <div className="app-container real-map-pdp-page">
    ...
    <main className="real-map-pdp-layout">
      <section className="real-map-pdp-panel">
        ...
        {result && <div className="real-pdp-stats"><strong>Optimization Result</strong><span>Drivers: {drivers.length} · Orders: {orders.length}</span><span>Assigned: {orders.length - result.unassigned_orders.length} · Unassigned: {result.unassigned_orders.length}</span><b>{Number(result.total_distance_km || 0).toFixed(3)} km</b><small>Shortest path: {result.algorithm} · PDP solver: {result.solver}</small>{result.routes.map((route) => <div className="route-stat" key={route.driver_id}><div className="route-stat-header"><strong>Driver {route.driver_id}</strong><span>{Number(route.total_distance_km || 0).toFixed(3)} km · {route.stops.length} stops</span></div><div className="stop-sequence">{route.stops.length ? route.stops.map((stop, index) => <div className="stop-sequence-row" key={`${route.driver_id}-${stop.order_id}-${stop.type}-${index}`}><span className="stop-sequence-number">{index + 1}</span><span className={stop.type === 'pickup' ? 'stop-type pickup-text' : 'stop-type dropoff-text'}>{stop.type === 'pickup' ? 'Pickup' : 'Dropoff'} order {stop.order_id}</span><span className="stop-node">{stop.node}</span><span className="stop-distance">{stop.distance_km == null ? 'Distance unavailable' : `+${Number(stop.distance_km).toFixed(3)} km`}</span></div>) : <span>No assigned stops</span>}</div></div>)}</div>}
      </section>
